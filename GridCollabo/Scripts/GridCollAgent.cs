...
public class GridCollAgent : Agent
{
    [Header("Specific to GridWorld")]
    private GridCollAcademy academy;
    ...
    {
        academy = FindObjectOfType(typeof(GridCollAcademy)) as GridCollAcademy;
    }
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        ...
        if (blockTest.Where(col => col.gameObject.CompareTag("wall")).ToArray().Length == 0)
        {
            ...
            if (blockTest.Where(col => col.gameObject.CompareTag("goal")).ToArray().Length == 1)
            {
                ...
                if (academy.removeGoal(Goal) == 0)
                {
                    Done();
                }
                SetReward(1f);
            }
            else if (blockTest.Where(col => col.gameObject.CompareTag("pit")).ToArray().Length == 1)
            {
                Done();
                SetReward(-1f);
            }
